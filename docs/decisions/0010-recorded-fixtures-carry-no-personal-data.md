...
A real response holds more than the character. It names the account that owns it, by username and user id. It links that account's chosen avatar, frame, and backdrop images. When the character is in a campaign, it also lists the campaign's name, its description, the game master's username, and every other player's username, character name, and avatar.
...
The tests do not need any of this data. The tool reads a character's classes, abilities, modifiers, actions, spells, and inventory. It never reads the owner, the decorations, or the campaign.

## Decision

Every recorded D&D Beyond response is blanked before it is committed. The owner's username becomes an empty string, and the user id becomes 0. The campaign becomes empty. Every avatar, frame, and backdrop field is emptied. The display theme colour stays, because it identifies no one.
