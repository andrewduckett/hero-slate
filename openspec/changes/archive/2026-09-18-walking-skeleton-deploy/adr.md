...
ADR review completed for this change. This is the first change in a greenfield repo, so no ADR directory existed. We established `docs/adr/` with a template and recorded the two durable, costly-to-reverse decisions from design.md. Two other decisions do not clear the ADR bar: fixing identity as the stable schema with provisional other fields, and the pure descriptor-line function. Each is cheap to reverse or owned by the specs.

## In-Force ADRs Reviewed

- None existed before this change. `docs/adr/` was created here, seeded with `0000-template.md`.
