...
We use SvelteKit built with `@sveltejs/adapter-static`, which emits pure static assets with no server-side Worker code and no server functions.
...
We ship no Worker code today — the site deploys to Cloudflare Workers static assets with no `main` script, so it is assets-only. It costs the same and has no server cold starts. We accept more setup now than a small router needs, because it removes a later migration.

### Render via SPA fallback with a runtime fetch

We set `ssr = false` and prerender the app shell. `adapter-static` emits a fallback page. On Cloudflare Workers static assets, `assets.not_found_handling: "single-page-application"` serves that shell (HTTP 200) for every clean path with no matching asset. The client then resolves the id and fetches the character YAML at runtime through the provider. We do not prerender one HTML page per character. Real assets take priority over the fallback, so a present `/characters/<id>.yaml` still returns the file. (A Pages-style `_redirects` rule `/* /index.html 200` is not used: Workers static assets rejects it as an infinite loop, since `/index.html` would re-match `/*`.)
...
- Rollback is a Cloudflare Workers redeploy of the previous build (`wrangler` redeploy/rollback), or reverting the branch before merge. No data or schema is at stake.
