...
- Scaffold a Vite + SvelteKit app built with `adapter-static`. The site runs fully static this release — SPA fallback, no worker, no server functions. Going dynamic later swaps the adapter, not the framework.
...
- Configure the Cloudflare Pages build and a `_redirects` SPA fallback so `/sunny` loads on the live site.
...
- `deployment`: Serve the app as a static site on Cloudflare Pages with SPA fallback so clean paths resolve.
...
- New deploy config: `adapter-static` fallback page plus a `_redirects` rule (`/* /index.html 200`) in `static/`.
