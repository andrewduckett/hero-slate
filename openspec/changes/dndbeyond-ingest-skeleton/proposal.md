...
- A new project-level agent skill, `.claude/skills/dndbeyond-to-slate/`, guides the Author from a D&D Beyond character URL or ID to a new `static/characters/<id>.yaml`.
...
- The `digest` command reports each failure as a distinct exit code with a one-line message:
  - a private character: the message tells the Author to make it public and retry
  - a character that does not exist, or an unreadable URL or ID
