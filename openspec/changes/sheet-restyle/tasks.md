...
- [ ] 2.1 Write a failing test that reads `src/lib/theme/fonts.css`. It asserts that every `@font-face` source is a root-relative `/fonts/` path and names no other host. It also asserts that each face sets `font-display: swap`, and that each `.woff2` file and its `OFL.txt` exist in `static/fonts/`. Verify it fails.
- [ ] 2.2 Download the latin-subset variable `.woff2` files for Baloo 2 and Nunito from Fontsource, with each family's `OFL.txt`, into `static/fonts/`. Verify the files exist and their licences are SIL OFL 1.1.
...
- [ ] 2.4 Run `npm run build` and verify that `build/fonts/` contains both `.woff2` files and both `OFL.txt` files.
