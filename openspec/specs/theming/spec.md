...
Gives a character a readable accent color from a fixed named palette, and switches the sheet between light and dark by device preference or by the player's stored choice. Dark is the resting default when neither applies. A child must be able to read the sheet in any room light, and config authors pick colors by name, never by raw hex.
